...
class Pricing_Services extends Widget_Base {
	...
	public function get_title() {
		return __( 'Pricing Services', 'imaginem-blocks' );
	}
	...
	protected function _register_controls() {
		$this->start_controls_section(
			'section_title',
			[
				'label' => __( 'Pricing Services', 'imaginem-blocks' ),
			]
		);
		...
		$this->add_control(
			'featured',
			[
				'label' => __( 'Featured', 'imaginem-blocks' ),
		...
		$this->add_control(
			'contentrichtext',
			[
				'label' => __( 'Services', 'imaginem-blocks' ),
				'type' => Controls_Manager::WYSIWYG,
				'placeholder' => __( 'Enter your services', 'imaginem-blocks' ),
				'default' => __( 'Add services text', 'imaginem-blocks' ),
			]
		);
	...
	protected function render() {
		...
		$shortcode = '[pricing_table service="true" columns="1"][pricing_column link="'.$url.'" link_target="'.$url_target.'" button_text="'.htmlspecialchars($settings['button_text']).'" title="'.htmlspecialchars($settings['title']).'" featured="'.$settings['featured'].'" currency="'.htmlspecialchars($settings['pricesymbol']).'" price="'.$settings['price'].'" fraction="'.$settings['fraction'].'" duration="'.htmlspecialchars($settings['duration']).'"]'.$service_shortcode.'[/pricing_column][/pricing_table]';
	...
	public function wpml_widgets_to_translate_filter( $widgets ) {
		$widgets[ $this->get_name() ] = [
			...
			'fields'     => [
				...
				[
					'field'       => 'pricesymbol',
					'type'        => __( 'Price Symbol', 'imaginem-blocks' ),
					'editor_type' => 'LINE'
				],
				[
					'field'       => 'price',
					'type'        => __( 'Price', 'imaginem-blocks' ),
					'editor_type' => 'LINE'
				],
				[
					'field'       => 'fraction',
					'type'        => __( 'Faction', 'imaginem-blocks' ),
					'editor_type' => 'LINE'
				],
				[
					'field'       => 'duration',
					'type'        => __( 'Duration', 'imaginem-blocks' ),
					'editor_type' => 'LINE'
				],
				[
					'field'       => 'button_text',
				...
				[
					...
					'type'        => __( 'Services', 'imaginem-blocks' ),
					'editor_type' => 'VISUAL'
				],
			],
		];

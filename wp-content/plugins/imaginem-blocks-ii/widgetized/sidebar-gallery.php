...
class mtheme_SidebarGallery_Widget extends WP_Widget {
	function __construct() {
		$widget_ops = array('classname' => 'widget_mtheme_sidebar_gallery', 'description' => 'Generates an image gallery.' );
		parent::__construct('mtheme_sidebar_gallery', __('blacksilver Gallery Widget'), $widget_ops);
	}
		...
		if ( !empty( $title ) ) { 
			...
			switch ($listchoice) {
				...
				case "portfolio" :
					...
					$thumbnail_image_id = get_post_thumbnail_id(($post->ID), 'blacksilver-gridblock-tiny'); 
					$thumbnail_image_url = wp_get_attachment_image_src($image_id,'blacksilver-gridblock-tiny');  
				...
				case "page" :
					if ( function_exists('blacksilver_get_custom_attachments') ) {
						$filter_image_ids = blacksilver_get_custom_attachments ( $thepage );
					}

					if ( isSet($filter_image_ids) && isSet($thepage) ) {
						...
						$thumbnail_imagearray = wp_get_attachment_image_src( $attatchmentID , 'blacksilver-gridblock-tiny', false);
						...
						?>
							...
							if ($clickaction=="lightbox") { ?>
								<a rel="prettyPhoto[<?php echo $args['widget_id']; ?>]" href="<?php echo $imageURI; ?>">
								<?php
								} else {
								?>
				...
				case "category" :
					...
					$thumbnail_image_id = get_post_thumbnail_id(($post->ID), 'blacksilver-gridblock-tiny'); 
					$thumbnail_image_url = wp_get_attachment_image_src($thumbnail_image_id,'blacksilver-gridblock-tiny'); 
					$thumbnail_image_url = $thumbnail_image_url[0];
					...
					?>
						...
						if ($clickaction=="lightbox") { ?>
							<a rel="prettyPhoto[<?php echo $args['widget_id']; ?>]" href="<?php echo $lightbox_url; ?>">
							<?php
							} else {
							?>
							<a href="<?php the_permalink() ?>">
...
function mtheme_SidebarGallery_Widget_register_widgets() {
	register_widget( 'mtheme_SidebarGallery_Widget' );
}
add_action( 'widgets_init', 'mtheme_SidebarGallery_Widget_register_widgets' );
?>
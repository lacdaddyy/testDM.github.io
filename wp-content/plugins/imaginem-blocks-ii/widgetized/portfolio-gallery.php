...
class mtheme_PortfolioGallery_Widget extends WP_Widget {
	function __construct() {
		...
		parent::__construct('mtheme_portfolio_gallery', __('blacksilver Portfolio Gallery Widget'), $widget_ops);
	}
	...
	public function widget($args, $instance) {
		...
		$title = empty($instance['title']) ? ' ' : apply_filters('widget_title', $instance['title']);
			...
			<?php
					...
					$thumbnail_image_id = get_post_thumbnail_id(( get_the_id() ), 'blacksilver-gridblock-square-big'); 
					$thumbnail_image_url = wp_get_attachment_image_src($image_id,'blacksilver-gridblock-square-big');  
					$thumbnail_image_url = $thumbnail_image_url[0];

					$srcset      = wp_get_attachment_image_srcset( $thumbnail_image_id, 'blacksilver-gridblock-square-big' );
					$srcsetsizes = wp_get_attachment_image_sizes( $thumbnail_image_id, 'blacksilver-gridblock-square-big' );
					...
					?>
						...
						<li class="<?php echo $class_string; ?> lazyload-wrapper">
						<?php

						if ($clickaction=="lightbox" && !post_password_required() ) { ?>
							<a class="lightbox-active lightbox-image" data-exthumbimage="<?php echo esc_attr($thumbnail_image_url); ?>" data-thumbnail="<?php echo esc_url($lightbox_url); ?>" data-src="<?php echo esc_url($lightbox_url); ?>" href="<?php echo esc_url($lightbox_url); ?>">
							<?php
							} else {
							?>
						...
						?>
								...
								$fallback_image = blacksilver_get_placeholder_image('blacksilver-gridblock-square-big');
								...
								$img_alt = blacksilver_get_alt_text($image_id);
								echo '<img '.$fallback_src.$data_src.'src="'. esc_url( $have_image_url ) .'" data-srcset="'.esc_attr($srcset).'" sizes="'.esc_attr($srcsetsizes).'" alt="'. esc_attr( $img_alt ) .'" class="'.$class.'"/>';
								?>
							</a>
						</li>			
...
function mtheme_PortfolioGallery_Widget_register_widgets() {
	register_widget( 'mtheme_PortfolioGallery_Widget' );
}
add_action( 'widgets_init', 'mtheme_PortfolioGallery_Widget_register_widgets' );
?>
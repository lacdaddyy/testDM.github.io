	...
	public function register_controls($element, $args) {
		...
		$element->add_control(
			'playlist_list',
				[
					...
					'condition' => ['sr_music_cpt_toggle' => 'yes',],
				]
		);
		...
		public function renderfnc($button) {
			 ...
			 if( 'button' === $button->get_name() ) {
			    ...
			    if( $settings['sr_music_cpt_toggle'] =="yes" && $settings['playlist_list'] ) {
					...
					$audiocall = "javascript:IRON.sonaar.player.setPlayer({id:" . $display_playlist_ar . "})";
					//print_r($audiocall);					

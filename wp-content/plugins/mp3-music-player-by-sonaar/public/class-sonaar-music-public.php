...
class Sonaar_Music_Public {
	...
	public function enqueue_styles() {
		...
		if ( function_exists( 'run_sonaar_music_pro' ) ){
			...
			$data .= ( $fontStickyPlayer['font-family'] !== '' && !strpos($fontStickyPlayer['font-family'], '_safe_') )?'@import url(//fonts.googleapis.com/css?family='. $fontStickyPlayer['font-family'] .');':'';
			...
			$data .= ( $fontStickyPlayer['color'] !== '' )? '#sonaar-player{ color:' . $fontStickyPlayer['color'] . ';}' :'';
	...
	public function enqueue_scripts() {
		...
		wp_register_script( 'sonaar-music-mp3player', plugin_dir_url( __FILE__ ) . 'js/iron-audioplayer/iron-audioplayer.js', array( 'jquery','sr-scripts', 'sonaar-music' ,'moments', 'wave' ), $this->version, true );

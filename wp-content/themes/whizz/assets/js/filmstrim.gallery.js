...
(function ($) {
	...
	function filmStrimGallery() {
		$('.filmstrim-gallery').each(function () {
			var thisGallery = $(this),
				thisImages = thisGallery.find('.image-wrap'),
				loop = thisGallery.data('loop') == true,
				autoplay = thisGallery.data('autoplay') == true,
				direction = thisGallery.data('direction'),
				autospeed = parseInt(thisGallery.data('autospeed'), 10),
				slidespeed = parseInt(thisGallery.data('slidespeed'), 10),
				first_slide = parseInt(thisGallery.data('first_slide'), 10),
				hoverpause = parseInt(thisGallery.data('hoverpause'), 10) == true,
				itemMargin = thisGallery.data('slide_margins') + 'px',
				prevButton,
				nextButton,
				currentPos;


			if (first_slide < 0 || first_slide > thisImages.length) {
				first_slide = 0;
			} else {
			...
			/*Controls*/
			if (thisGallery.data('controls') == false) {
				prevButton = false;
				nextButton = false;
				thisGallery.closest('.filmstrim-gallery-outer').find('.swipe-btn').addClass('hidden');
			} else {
				prevButton = '.swipe-btn.prev';
				nextButton = '.swipe-btn.next';
			}
				
			thisGallery.carouFredSel({
				direction: direction,
				circular: loop,
				infinite: 0,
				width: 'auto',
			...
			thisImages.each(function () {
				var thisItem = $(this),
					thisImageHeight,
					thisImage = thisItem.find('.image'),
					thisImageSrc = 'url(\'' + thisImage.attr('src') + '\')';
				
				if (windowWidth > 767) {
					thisGallery.closest('.portfolio-single-content').removeClass('no-marg');

					thisImageHeight = $(this).closest('.filmstrip-albums') ?  (windowHeight - headerHeight - 100) : (windowHeight - headerHeight - footerHeight - 30);

					thisImage.height(thisImageHeight).css('visibility', 'hidden');	
					thisItem.width(thisImage.width()).height(thisImage.height()).css({
						'background-image': thisImageSrc,
						'margin-right': itemMargin
					});
				} else {
					thisGallery.closest('.portfolio-single-content').addClass('no-marg');
					thisImageHeight = windowHeight - headerHeight;
					thisImage.height(thisImageHeight).css('visibility', 'hidden');
					thisItem.width(thisImage.width()).height(thisImage.height()).css({
						'background-image': thisImageSrc,
						'margin-right': '0px'
		...
		$("body").keydown(function(e) {
		  if(e.keyCode == 37) { // left
		    $(".swipe-btn.prev").trigger('click');
		  }
		  else if(e.keyCode == 39) { // right
		    $(".swipe-btn.next").trigger('click');
		  }
		});
	}
    ...
    if ( (Modernizr.touchevents && typeof window.disable_mobile_panning !== "undefined" && window.disable_mobile_panning == true)) {
        imageScaling = 'fit';
    }
     ...
     function eventHandlers() {
        $(window).on('debouncedresize', resizeFullView);
        $(window).on('resize', function(){
        	resizeFullView();
        });
    ...
    function panFullview() {
        $('.fullview__image img').each(function(i, obj) {
            ...
            if (Modernizr.touchevents) {
                ...
                if (imgHeight > windowHeight) {
                	var pos = (windowHeight / 2 - latestKnownMouseY) * (imgHeight - windowHeight) / windowHeight;
                    TweenMax.to($img, 0, {
                        y: pos
                    });
                }
    ...
    function addImageToFullView($source) {
        ...
        $image.attr('src', $source.data('srcfull'))
              .prependTo($target); 
    ...
    function bindVertToHorScroll() {
        /*if ( !$html.hasClass('is--ie-le10')) {
            ...
            $('html, body, .fullview__img').bind('mousewheel', vertToHorScroll);
    ...
    function showPrev(e) {
        var $items = $film.find('.image-wrap'),
            items = $items.length;
    ...
    $('.filmstrim-gallery .image-wrap').on('click', function(e){
    	$('.fullview').addClass('fullview--visible');
    	showFullView(this);
    	//$('.fullview').find('.fullview__image').html( '<img src="' + $(this).find('img').attr('src') + '">' );
    });

     $('.fullview__close').on('click', function(){
     	$('.fullview').removeClass('fullview--visible');
     });

    function setCurrent($current) {
        $film.find('.image-wrap').removeClass('portfolio__item--active');
        $current.addClass('portfolio__item--active');
        $('.portfolio__position').text($current.data('count') + 1 + ' ' + objectl10n.tCounter + ' ' + $film.find('.image-wrap').not('.portfolio__item--clone').length);
    }
